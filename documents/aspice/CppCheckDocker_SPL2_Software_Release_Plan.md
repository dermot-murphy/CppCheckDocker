...
| v1.03 | 2026-08-13 | Dermot Murphy | §5.1 step 4 wording refreshed: `Publish-Wiki` now publishes both the User Guide and the ASPICE CL2 document set grouped by ISO/IEC 33020 process category (issue #43). |
...
4. `Publish-Wiki` job (see build.yml) regenerates the repository wiki from `documents/user_guide/` (User Guide section) and `documents/aspice/` (ASPICE CL2 section, grouped by ISO/IEC 33020 process category and walked recursively so `documents/aspice/records/` release evidence is included). The job runs regardless of the paths filter — the wiki tracks doc changes independently of image releases. Sidebar, Home, and ASPICE-Index are regenerated by `scripts/generate_wiki.py` on every run so the wiki structure is fully authoritative from the repo.
